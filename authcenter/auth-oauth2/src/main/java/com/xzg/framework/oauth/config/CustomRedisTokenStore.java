package com.xzg.framework.oauth.config;

import com.xzg.framework.oauth.constant.SecurityConstants;
import com.xzg.framework.oauth.properties.AuthProperties;
...
public class CustomRedisTokenStore implements TokenStore {
    ...
    public CustomRedisTokenStore(RedisConnectionFactory connectionFactory, AuthProperties authProperties) {
    ...
    public OAuth2Authentication readAuthentication(OAuth2AccessToken token) {
        ...
        //是否开启token续签
        boolean isRenew = authProperties.isEnable();
        if (isRenew && auth2Authentication != null) {
            ...
            if (checkRenewClientId(clientAuth.getClientId())) {
                ...
                if (validitySeconds > 0) {
                    ...
                    if (expiresRatio <= authProperties.getTimeRatio()) {
                        //更新AccessToken过期时间
